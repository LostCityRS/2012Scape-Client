package com.jagex;

import deob.ObfuscatedName;

@ObfuscatedName("aez")
public class class833 extends class800 implements class464 {

	@ObfuscatedName("aez.t")
	public int field9804 = 0;

	@ObfuscatedName("aez.l")
	public int[] field9801;

	@ObfuscatedName("aez.f")
	public int[] field9803;

	@ObfuscatedName("aez.d")
	public int field9802 = 0;

	@ObfuscatedName("aez.u(Laet;I)V")
	public void method15624(class814 arg0) {
		while (true) {
			int var2 = arg0.method15220();
			if (var2 == 0) {
				return;
			}
			this.method15621(arg0, var2);
		}
	}

	@ObfuscatedName("aez.j(Laet;II)V")
	public void method15621(class814 arg0, int arg1) {
		if (arg1 == 2) {
			this.field9802 = arg0.method15239() * -1038843161;
		} else if (arg1 == 4) {
			this.field9804 = arg0.method15220() * -384214557;
			this.field9801 = new int[this.field9804 * 1921299403];
			this.field9803 = new int[this.field9804 * 1921299403];
			for (int var3 = 0; var3 < this.field9804 * 1921299403; var3++) {
				this.field9801[var3] = arg0.method15239();
				this.field9803[var3] = arg0.method15239();
			}
		}
	}
}
