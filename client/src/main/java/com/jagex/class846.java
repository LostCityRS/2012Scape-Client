package com.jagex;

import deob.ObfuscatedName;

@ObfuscatedName("afh")
public class class846 extends class537 {

	public class846(class815 arg0) {
		super(arg0);
	}

	public class846(int arg0, class815 arg1) {
		super(arg0, arg1);
	}

	@ObfuscatedName("afh.z(B)V")
	public void method15734() {
		if (this.field6462.field9641.method15741() == 2 && this.field6461 * -402581687 == 2) {
			this.field6461 = 2118879993;
		}
		if (this.field6461 * -402581687 < 0 || this.field6461 * -402581687 > 2) {
			this.field6461 = this.method11386() * 2118879993;
		}
	}

	@ObfuscatedName("afh.u(I)I")
	public int method11386() {
		return 2;
	}

	@ObfuscatedName("afh.j(IB)I")
	public int method11387(int arg0) {
		return 1;
	}

	@ObfuscatedName("afh.s(II)V")
	public void method11389(int arg0) {
		this.field6461 = arg0 * 2118879993;
	}

	@ObfuscatedName("afh.n(B)I")
	public int method15735() {
		return this.field6461 * -402581687;
	}
}
